...
<tr>
		<td>
				<input type="text" name="{{ $name_col }}[]" value="{{ !empty($name) ? $name : '' }}"
		...
		<td>
				...
				<div class="input-group percent_field @if ($amount_type != 'percent') hide @endif">
						<input type="text" name="{{ $percent_col }}[]" value="{{ @num_format($percent) }}"
								class="form-control input-sm input_number percent" readonly>
		...
		@if ($type == 'allowance' && $shortname == 'overtime')
				<td>
						...
						@endphp
						<input type="text" name="payrolls[{{ $employee }}][overtime_hours][]" value="{{ $overtime_value }}"
								class="form-control input-sm value_field input_number overtime_hours" {{ $readonly }}>
				</td>
		@elseif ($type == 'allowance' && $shortname != 'overtime')
				<td></td>
				<input type="hidden" name="payrolls[{{ $employee }}][overtime_hours][]" value="{{ $overtime_value }}"
						class="">
		@endif
    ...
    @endif --}}
		...
		<td>
				@php
						$readonly = $amount_type == 'percent' ? 'readonly' : '';
				@endphp
				<input type="text" name="{{ $val_col }}[]" value="{{ !empty($value) ? @num_format((float) $value) : 0 }}"
						class="form-control input-sm value_field input_number {{ $val_class }}" {{ $readonly }}>
		</td>
