<table>
    ...
    <tr>
        ...
        <td>{{ $salaryYear }}</td>
        <td>{{ $salaryMonth }}</td>
        <td>{{ number_format(round($totalSalary, 3), 3, '.', '') }}</td> 
    ...
    @foreach ($transactionPayrolls as $transactionPayroll)
        <tr>
            ...
            <td>{{ number_format(round($transactionPayroll->final_total, 3), 3, '.', '') }}</td> 
            <td>{{ number_format(round($transactionPayroll->total_before_tax, 3), 3, '.', '') }}</td>
            <td>{{ $transactionPayroll->extra_hours }}</td>
            <td>{{ number_format(round($transactionPayroll->essentials_allowances, 3), 3, '.', '') }}</td>
            <td>{{ number_format(round($transactionPayroll->essentials_deductions, 3), 3, '.', '') }}</td>
            <td>{{ number_format(round($transactionPayroll->social_security_deductions, 3), 3, '.', '') }}</td>
            <td></td>
        </tr>
    @endforeach
</table>